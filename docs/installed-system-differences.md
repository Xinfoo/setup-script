...
新版的不同之处在于，它先要求用户明确确认 shim 包的可信来源与可用性，再在 Live 环境的私有 tmpfs 中检查包名和结构，并提取、检查三个 EFI 文件。这些自动检查不认证软件包的来源，也不审查其安装脚本或 hook。之后只把该包的快照副本临时放入目标 `/root`，安装完成后删除。旧版则直接复制输入包和整个密钥目录。两版在 Pacman 包数据库及 `/usr/share/shim-signed/` 内容上没有预期差异。
